...
export function renderNavbar(cv: { name: string; lastName: string; title: string }): HTMLElement {
  ...
  for (const node of renderFormattedText(cv.name)) {
    nameEl.appendChild(node);
  }
  logo.appendChild(nameEl);

  const titleEl = document.createElement('span');
  titleEl.className = 'logo-title';
  for (const node of renderFormattedText(cv.title)) {
